...
仓库目前面向 DeepSeek Harness `0.1.5-alpha.1`，暂时通过 GitHub 源码安装，尚未发布 npm 版本。
...
CLI 必须为 `0.1.5-alpha.1`；默认 `dsh` 版本不匹配时，通过 `DSH_CMD` 指定对应可执行文件。测试使用临时 profile，并将基础和 Web bundle 固定到同一版本。
