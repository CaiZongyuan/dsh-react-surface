...
The repository currently targets DeepSeek Harness `0.1.5-rc.2` and is installed from source. It is not an npm release yet.
...
The browser lane requires a DSH CLI matching `0.1.5-rc.2`; set `DSH_CMD` to its executable path when the default `dsh` is a different version. Its temporary profile pins the base and Web bundles to the same cohort.
