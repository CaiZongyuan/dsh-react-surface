...
The Client mounts through official `shell.overlay` and `sidebar.footer.action` Slots. Split layouts require geometry from the DSH frame. The Host Adapter resolves official semantic pane attributes (including `data-rightbar-col`) first, then the tested `0.1.5-rc.1` frame structure as a centralized fallback.
